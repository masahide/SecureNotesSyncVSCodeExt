...
export class SyncService implements ISyncService {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        "LocalObjectManager not available. Ensure it is registered in the container.",
      );
    }
  }
  ...
  async initializeNewStorage(): Promise<boolean> {
    try {
      ...
      const initialIndex = await this.localObjectManager.generateInitialIndex(
        this.syncOptions,
      );
      ...
      await this.localObjectManager.saveWsIndexFile(
        initialIndex,
        this.syncOptions,
      );
  ...
  async importExistingStorage(): Promise<boolean> {
    try {
      ...
      const remoteIndex = await this.localObjectManager.loadRemoteIndex(
        this.syncOptions,
      );
      await this.localObjectManager.saveWsIndexFile(
        remoteIndex,
      ...
      // ファイルをワークスペースに展開
      const emptyIndex = await this.localObjectManager.generateEmptyIndex(
        this.syncOptions,
      );
  ...
  async performIncrementalSync(): Promise<boolean> {
    try {
      ...
      const hasRemoteChanges =
        await this.dependencies.storageProvider.pullRemoteChanges(
          currentBranch,
        );
      logMessage(`Remote storage changes detected: ${hasRemoteChanges}`);

      // 3. 増分同期処理を実行（直前のpull結果を渡す）
      const syncResult = await this.performTraditionalIncrementalSync(
        this.syncOptions,
        currentBranch,
        hasRemoteChanges,
        syncTimestamp,
   ...
   */
  private async performTraditionalIncrementalSync(
    options: SyncOptions,
    ...
    // 2. 新しいローカルインデックスを生成
    const newLocalIndex = await this.localObjectManager.generateLocalIndexFile(
      previousIndex,
      options,
    );
    ...
    logMessage(
      `Final update decision: hasLocalChanges=${hasLocalChanges}, hasRemoteUpdates=${hasRemoteUpdates}, updated=${updated}`,
    );

    // 4. リモート更新がある場合は競合検出・解決
    if (hasRemoteUpdates) {
      logMessage("Processing remote updates...");
      const mergeResult = await this.handleRemoteUpdates(
        previousIndex,
        newLocalIndex,
        options,
        syncTimestamp,
      ...
      updated = true;
      logMessage(
        `After remote merge: finalIndex UUID=${finalIndex.uuid}, updated=${updated}`,
      );
    }

    // 5. ファイルを暗号化保存
    const filesUpdated = await this.localObjectManager.saveEncryptedObjects(
      finalIndex.files,
      previousIndex,
      options,
    );
    ...
    if (updated) {
      logMessage(
        `Proceeding with finalization: finalIndex UUID=${finalIndex.uuid}`,
      );
  ...
  private async handleRemoteUpdates(
    ...
    logMessage(
      `Remote index loaded: UUID=${remoteIndex.uuid}, files=${remoteIndex.files.length}`,
    );
    ...
    logMessage(
      `Merged index created: UUID=${mergedIndex.uuid}, files=${mergedIndex.files.length}`,
    );
  ...
  private async finalizeSync(
    ...
    const previousIndex = await this.localObjectManager.loadWsIndex(options);
    logMessage(
      `finalizeSync: Before update - previousIndex: ${previousIndex.uuid} (${previousIndex.files.length} files), finalIndex: ${finalIndex.uuid} (${finalIndex.files.length} files)`,
    );
    ...
    logMessage(
      `finalizeSync: After saveWsIndexFile - finalIndex: ${finalIndex.uuid}`,
    );

    // ファイル変更を反映（更新前のインデックスと比較）
    logMessage(
      `finalizeSync: Reflecting file changes - previousIndex: ${previousIndex.uuid} (${previousIndex.files.length} files), finalIndex: ${finalIndex.uuid} (${finalIndex.files.length} files), forceCheckout: false`,
    );
    await this.localObjectManager.reflectFileChanges(
      previousIndex,
      finalIndex,
