...
export class SecureNotesLayoutManager implements ISecureNotesLayoutManager {
  ...
  constructor(
    ...
    this.remotesDirUri = vscode.Uri.joinPath(
      this.workspaceUri,
      ".secureNotes",
      "remotes",
    );
  ...
  async prepareRemotesLayout(options?: {
    ...
    if (clearExisting) {
      try {
        await this.fileSystem.delete(this.remotesDirUri, {
          recursive: true,
          useTrash: false,
        });
      } catch {
    ...
    const gitattributesUri = vscode.Uri.joinPath(
      this.remotesDirUri,
      ".gitattributes",
    );
