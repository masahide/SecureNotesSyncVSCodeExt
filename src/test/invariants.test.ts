...
const initialWorkspaceFolder = {
  uri: vscode.Uri.file(initialWsDir),
  name: "ws1",
  index: 0,
} as vscode.WorkspaceFolder;
Object.defineProperty(vscode.workspace, "workspaceFolders", {
  value: [initialWorkspaceFolder],
  writable: true,
  configurable: true,
});
...
suite("Invariant Tests", () => {
  test("LocalObjectManager uses fixed workspace after construction", async () => {
    ...
    const anotherWorkspaceFolder = {
      uri: vscode.Uri.file(anotherDir),
      name: "ws2",
      index: 0,
    } as vscode.WorkspaceFolder;
    Object.defineProperty(vscode.workspace, "workspaceFolders", {
      value: [anotherWorkspaceFolder],
      writable: true,
      configurable: true,
    });
    ...
    assert.strictEqual(
      path.dirname(path.dirname(refs1.fsPath)),
      path.dirname(path.dirname(refs2.fsPath)),
      "Refs directory should remain under initial workspace",
    );
  });

  test("GitHubSyncProvider requires workspaceUri and keeps it immutable in practice", async () => {
    const workspaceUri = vscode.Uri.file(initialWsDir);
    const fileSystem = new VsCodeFileSystem();
    const gitClient = new NodeGitClient(findGitExecutable());
    const layoutManager = new SecureNotesLayoutManager(
      workspaceUri,
      fileSystem,
    );
    ...
    assert.strictEqual(
      internalWs.fsPath,
      initialWsDir,
      "workspaceUri should equal constructor arg",
    );
  });
});
