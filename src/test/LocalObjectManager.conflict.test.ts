...
suite("LocalObjectManager conflict path helper", () => {
  test("buildConflictFilePath で共通フォーマットを提供すること", () => {
    ...
    const localPath = manager.buildPath("conflict-local", "notes/sample.md", timestamp);
    const remotePath = manager.buildPath("conflict-remote", "notes/sample.md", timestamp);
