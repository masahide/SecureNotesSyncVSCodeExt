...
suite("Integration Test Suite", () => {
  ...
  test("初回同期 - 空のリモートリポジトリ", async () => {
    ...
    const tempRepoPath = require("path").join(
      require("os").tmpdir(),
      `integration-empty-repo-${Date.now()}.git`,
    );
    const syncService = createTestSyncService(
      `file://${tempRepoPath}`,
      mockContext as any,
    );
    ...
    try {
      ...
      assert.ok(
        fs.existsSync(remotesDir),
        "remotesディレクトリが作成されること",
      );
    } catch (error) {
  ...
  test("既存リポジトリからの復元", async () => {
    ...
    const tempRepoPath = require("path").join(
      require("os").tmpdir(),
      `integration-existing-repo-${Date.now()}.git`,
    );
    ...
    try {
      ...
      execSync(`git clone ${tempRepoPath} .`, {
        cwd: workDir,
        stdio: "ignore",
      });
  ...
  test("複数ファイルの同期処理", async () => {
    ...
    const testFiles = [
      ...
      {
        path: "config.json",
        content: '{"version": "1.0.0", "name": "test-project"}',
      },
    ];
    ...
    try {
      ...
      const tempRepoPath = path.join(
        require("os").tmpdir(),
        `integration-multi-files-${Date.now()}.git`,
      );
      const syncService = createTestSyncService(
        `file://${tempRepoPath}`,
        mockContext as any,
  ...
  test("暗号化キー不正時の処理", async () => {
    ...
    for (const scenario of invalidKeyScenarios) {
      ...
      const tempRepoPath = require("path").join(
        require("os").tmpdir(),
        `integration-invalid-key-${Date.now()}.git`,
      );
      const syncService = createTestSyncService(
        `file://${tempRepoPath}`,
        mockContext as any,
      ...
      try {
        ...
        console.log(
          `${scenario.name}: エラーが発生しなかった（初期段階では検出されない可能性）`,
        );
      } catch (error) {
    ...
    try {
      ...
      const tempRepoPath = require("path").join(
        require("os").tmpdir(),
        `integration-valid-key-${Date.now()}.git`,
      );
      const syncService = createTestSyncService(
        `file://${tempRepoPath}`,
        mockContext as any,
      ...
      const errorMessage = (error as Error).message || String(error);
      console.log(
        `Valid key test error (expected Git-related): ${errorMessage}`,
      );
      assert.ok(
  ...
  test("ワークスペース不正時の処理", async () => {
    ...
    const tempRepoPath = require("path").join(
      require("os").tmpdir(),
      `integration-workspace-error-repo-${Date.now()}.git`,
    );
    const syncService = createTestSyncService(
      `file://${tempRepoPath}`,
      mockContext as any,
