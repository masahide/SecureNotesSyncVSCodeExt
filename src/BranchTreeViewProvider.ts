...
export class BranchTreeViewProvider
  implements
    vscode.TreeDataProvider<BranchItem | IndexItem>,
    IBranchTreeViewProvider
{
   ...
   */
  private async getIndexHistoryOfBranch(
    branchItem: BranchItem,
  ): Promise<IndexItem[]> {
    const aesKey = await this.keyManagementService.getKey();
    if (!aesKey) {
    ...
    try {
      ...
      logMessage(
        `getIndexHistoryOfBranch loadIndex error: ${err?.message ?? err}`,
      );
    }
