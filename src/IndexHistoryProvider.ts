  ...
  implements vscode.TreeDataProvider<IndexItem>
{
  ...
  async getChildren(element?: IndexItem): Promise<IndexItem[]> {
    ...
    if (element) {
      logMessage(
        "IndexHistoryProvider: getChildren - element exists, returning empty array",
      );
      return [];
    }

    const encryptionKey = await this.keyManagementService.getKey();
    if (!encryptionKey) {
      logMessage("IndexHistoryProvider: AES encryption key not found.");
      vscode.window.showErrorMessage(
        "AES encryption key not found. Please set it in the settings.",
      );
      return [];
    }
    ...
    try {
      ...
      logMessage(
        `IndexHistoryProvider: Workspace folder not found. ${error?.message ?? error}`,
      );
      vscode.window.showErrorMessage("No workspace folder open.");
      return [];
    }
    logMessage(
      `IndexHistoryProvider: Workspace folder: ${workspaceUri.fsPath}`,
    );
    ...
    try {
      ...
      logMessage(
        `IndexHistoryProvider: Found ${indexes.length} remote indexes.`,
      );

      indexes.sort((a, b) => b.timestamp - a.timestamp);
      logMessage(
        "IndexHistoryProvider: Indexes sorted by timestamp descending.",
      );

      const recentIndexes = indexes.slice(0, 30);
      logMessage(
        `IndexHistoryProvider: Sliced to ${recentIndexes.length} recent indexes.`,
      );

      const indexItems = recentIndexes.map((index) => new IndexItem(index));
      logMessage(
        `IndexHistoryProvider: Mapped to ${indexItems.length} IndexItem objects.`,
      );
      return indexItems;
    } catch (error) {
...
class IndexItem extends vscode.TreeItem {
  constructor(public readonly indexFile: IndexFile) {
    super(
      new Date(indexFile.timestamp).toLocaleString(),
